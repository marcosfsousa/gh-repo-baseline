...
``templates/``, checks the guard passes, and then breaks the repo fourteen
...
class TestTheGuardPasses:
    ...
    def test_it_is_not_a_trivial_suite(self, boot, tmp_path):
        ...
        assert "23 tests collected" in result.stdout, result.stdout[-600:]
...
# ── The guard on fourteen broken repos ────────────────────────────────────────
#
