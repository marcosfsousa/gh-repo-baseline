...
- **`tests/test_guard.py`** — assembles a throwaway repo out of `templates/`,
  confirms the guard passes, then breaks it **seventeen ways** and confirms the
  ...
  required, CI told to skip the protected branch, its filter moved out of the
  parser's sight and its filter lent to it by a commented-out event below,
  a check required from a workflow the guard does not read,
  ruleset retargeted to another
